...
void getKernelVec(int ngauss, int *deg_fixe, double **kernel_vec, int usePCA, int fwKernel, int hwKernel, float *sigma_gauss, double *filter_x, double *filter_y, float **PCA) {
    /*****************************************************
    ...
    for (ig = 0; ig < ngauss; ig++) {
        for (idegx = 0; idegx <= deg_fixe[ig]; idegx++) {
            for (idegy = 0; idegy <= deg_fixe[ig]-idegx; idegy++) {
                ...
                kernel_vec[nvec] = kernel_vector(nvec, idegx, idegy, ig, &ren, usePCA, fwKernel, hwKernel, sigma_gauss, filter_x, filter_y, kernel_vec, PCA);
                nvec++;
            }
        }
    }
...
int fillStamp(stamp_struct *stamp, float *imConv, float *imRef, int rPixX, int rPixY, int verbose, int ngauss, int *deg_fixe, int hwKSStamp, int fwKSStamp, int hwKernel, int fwKernel, int bgOrder, int nCompKer, int kerOrder, int usePCA, double *filter_x, double *filter_y, float *temp, float **PCA, float fillVal, int *mRData) {
    /*****************************************************
    ...
    for (ig = 0; ig < ngauss; ig++) {
        for (idegx = 0; idegx <= deg_fixe[ig]; idegx++) {
            for (idegy = 0; idegy <= deg_fixe[ig]-idegx; idegy++) {
                ...
                xy_conv_stamp(stamp, imConv, nvec, ren, usePCA, fwKSStamp, fwKernel, hwKSStamp, hwKernel, rPixX, filter_x, filter_y, temp, PCA);
                ++nvec;
            }
        }
    }
    ...
    if (cutSStamp(stamp, imRef, fwKSStamp, hwKSStamp, fillVal, rPixX, mRData, verbose))
        return 1;
    ...
    build_matrix0(stamp, nCompKer, kerOrder, bgOrder, fwKSStamp);
    /* build stamp->scprod from stamp->vectors and imRef */
    build_scprod0(stamp, imRef, nCompKer, kerOrder, bgOrder, fwKSStamp, hwKSStamp, rPixX);
    
    return 0;
}

double *kernel_vector(int n, int deg_x, int deg_y, int ig, int *ren, int usePCA, int fwKernel, int hwKernel, float *sigma_gauss, double *filter_x, double *filter_y, double **kernel_vec, float **PCA) {
    /*****************************************************
    ...
    if (usePCA) {
        return kernel_vector_PCA(n, deg_x, deg_y, ig, ren, fwKernel, PCA, kernel_vec);
    }
...
double *kernel_vector_PCA(int n, int deg_x, int deg_y, int ig, int *ren, int fwKernel, float **PCA, double **kernel_vec) {
    /*****************************************************
     * Creates kernel sized entry for kernel_vec for each kernel degree 
     *   Mask of filter_x * filter_y, filter = exp(-x**2 sig) * x^deg 
     *   Subtract off kernel_vec[0] if n > 0
...
void xy_conv_stamp(stamp_struct *stamp, float *image, int n, int ren, int usePCA, int fwKSStamp, int fwKernel, int hwKSStamp, int hwKernel, int rPixX, double *filter_x, double *filter_y, float *temp, float **PCA) {
    /*****************************************************
    ...
    if (usePCA) {
        xy_conv_stamp_PCA(stamp, image, n, ren, fwKSStamp, hwKSStamp, hwKernel, fwKernel, rPixX, PCA);
        return;
    }
...
void xy_conv_stamp_PCA(stamp_struct *stamp, float *image, int n, int ren, int fwKSStamp, int hwKSStamp, int hwKernel, int fwKernel, int rPixX, float **PCA) {
    /*****************************************************
     * Called for each degree of convolution, ngauss by deg_gauss 
     * Each convolution is stored in stamp->vectors[n], imc here
     ******************************************************/
...
void fitKernel(stamp_struct *stamps, float *imRef, float *imConv, float *imNoise, double *kernelSol, 
               double *meansigSubstamps, double *scatterSubstamps, int *NskippedSubstamps, int nCompKer, int kerOrder, int bgOrder, int verbose, int nS, int *indx, int fwKSStamp, int hwKSStamp, int rPixX, int rPixY, char *figMerit, float kerSigReject, float statSig, int *mRData, float *temp, int ngauss, int *deg_fixe, int hwKernel, int fwKernel, int usePCA, double *filter_x, double *filter_y, float **PCA, float fillVal) {
    /*****************************************************
    ...
    double **wxy = (double **)malloc(nS*sizeof(double *));
    for (i = 0; i < nS; i++)
    ...
    build_matrix(stamps, nS, matrix, nCompKer, kerOrder, bgOrder, fwKSStamp, rPixX, rPixY, verbose, wxy);
    build_scprod(stamps, nS, imRef, kernelSol, nCompKer, kerOrder, bgOrder, fwKSStamp, hwKSStamp, rPixX, wxy);
    
    ludcmp(matrix, mat_size, indx, &d);
    lubksb(matrix, mat_size, indx, kernelSol);
    
    if (verbose>=2) fprintf(stderr, " Checking again\n");
    check = check_again(stamps, kernelSol, imConv, imRef, imNoise, meansigSubstamps, scatterSubstamps, NskippedSubstamps, nS, verbose, figMerit, kerSigReject, statSig, fwKSStamp, hwKSStamp, rPixX, rPixY, mRData, temp, nCompKer, kerOrder, bgOrder, ngauss, deg_fixe, hwKernel, fwKernel, usePCA, filter_x, filter_y, PCA, fillVal);
...
void build_matrix0(stamp_struct *stamp, int nCompKer, int kerOrder, int bgOrder, int fwKSStamp) {
    /*****************************************************
...
void build_scprod0(stamp_struct *stamp, float *image, int nCompKer, int kerOrder, int bgOrder, int fwKSStamp, int hwKSStamp, int rPixX) {
    /*****************************************************
...
double check_stamps(stamp_struct *stamps, int nS, float *imRef, float *imNoise, int nCompKer, int kerOrder, int bgOrder, int nCompTotal, int verbose, int *indx, double **check_mat, double *check_vec, double *check_stack, char *forceConvolve, char *figMerit, float kerSigReject, float statSig, int fwKSStamp, int hwKSStamp, int rPixX, int rPixY, int fwKernel, double **kernel_vec, double *kernel_coeffs, double *kernel, int *mRData, float *temp) {
    /*****************************************************
    ...
    sigma_clip(ks, nks, &kmean, &kstdev, 10, statSig);
    ...
    if ((strncmp(forceConvolve, "b", 1)==0)) {
        ...
        /* allocate weight matrix */
        double **wxy = (double **)calloc(nS, sizeof(double *));
        for (i = 0; i < nS; i++)
        ...
        build_matrix(testStamps, ntestStamps, matrix, nCompKer, kerOrder, bgOrder, fwKSStamp, rPixX, rPixY, verbose, wxy);
        build_scprod(testStamps, ntestStamps, imRef, testKerSol, nCompKer, kerOrder, bgOrder, fwKSStamp, hwKSStamp, rPixX, wxy);
        ludcmp(matrix, mat_size, indx, &d);
        lubksb(matrix, mat_size, indx, testKerSol);
        
        /* get the kernel sum to normalize figures of merit! */
        kmean = make_kernel(0, 0, testKerSol, rPixX, rPixY, nCompKer, kerOrder, fwKernel, kernel_vec, kernel_coeffs, kernel);
        ...
        for (i = 0; i < ntestStamps; i++) {
            ...
            getStampSig(&testStamps[i], testKerSol, imNoise, &sig1, &sig2, &sig3, fwKSStamp, hwKSStamp, rPixX, rPixY, mRData, figMerit, temp, statSig, nCompKer, kerOrder, bgOrder);
        ...
        sigma_clip(m1, mcnt1, &merit1, &sig1, 10, statSig);
        sigma_clip(m2, mcnt2, &merit2, &sig2, 10, statSig);
        sigma_clip(m3, mcnt3, &merit3, &sig3, 10, statSig);
...
void build_matrix(stamp_struct *stamps, int nS, double **matrix, int nCompKer, int kerOrder, int bgOrder, int fwKSStamp, int rPixX, int rPixY, int verbose, double **wxy) {
    /*****************************************************
     * Build overall matrix including spatial variations
     *****************************************************/
...
void build_scprod(stamp_struct *stamps, int nS, float *image, double *kernelSol, int nCompKer, int kerOrder, int bgOrder, int fwKSStamp, int hwKSStamp, int rPixX, double **wxy) {
    /*****************************************************
...
void getFinalStampSig(stamp_struct *stamp, float *imDiff, float *imNoise, double *sig, int fwKSStamp, int hwKSStamp, int rPixX, int *mRData) {
    int i, j, idx, nsig=0;
...
void getStampSig(stamp_struct *stamp, double *kernelSol, float *imNoise, double *sig1, double *sig2, double *sig3, int fwKSStamp, int hwKSStamp, int rPixX, int rPixY, int *mRData, char *figMerit, float *temp, float statSig, int nCompKer, int kerOrder, int bgOrder) {
    ...
    im = stamp->krefArea;
    /* background from fit */
    bg = get_background(xRegion, yRegion, kernelSol, nCompKer, kerOrder, bgOrder, rPixX, rPixY);
    /* temp contains the convolved image from fit, fwKSStamp x fwKSStamp */
    make_model(stamp, kernelSol, temp, rPixX, rPixY, nCompKer, kerOrder, fwKSStamp); 
    ...
    *sig3 = 0;
    ...
    if (strncmp(figMerit, "v", 1)!=0) {
         if (getStampStats3(temp, xRegion - hwKSStamp, yRegion - hwKSStamp, fwKSStamp, fwKSStamp,
                           &cSum, &cMean, &cMedian, &cMode, sig2, sig3, &cLfwhm, 0x0, 0xffff, 5, rPixX, mRData, statSig)) {
            *sig2 = -1;
            *sig3 = -1;
        }
...
char check_again(stamp_struct *stamps, double *kernelSol, float *imConv, float *imRef, float *imNoise, double *meansigSubstamps, double *scatterSubstamps, int *NskippedSubstamps, int nS, int verbose, char *figMerit, float kerSigReject, float statSig, int fwKSStamp, int hwKSStamp, int rPixX, int rPixY, int *mRData, float *temp, int nCompKer, int kerOrder, int bgOrder, int ngauss, int *deg_fixe, int hwKernel, int fwKernel, int usePCA, double *filter_x, double *filter_y, float **PCA, float fillVal) {
    /*****************************************************
    ...
    *NskippedSubstamps=0;
    ...
    for (istamp = 0; istamp < nS; istamp++) {
        ...
        if (stamps[istamp].sscnt < stamps[istamp].nss) {
            ...
            getStampSig(&stamps[istamp], kernelSol, imNoise, &sig1, &sig2, &sig3, fwKSStamp, hwKSStamp, rPixX, rPixY, mRData, figMerit, temp, statSig, nCompKer, kerOrder, bgOrder);
            ...
            if ((strncmp(figMerit, "v", 1)==0 && (sig1 == -1)) ||
                ...
                stamps[istamp].sscnt++;
                fillStamp(&stamps[istamp], imConv, imRef, rPixX, rPixY, verbose, ngauss, deg_fixe, hwKSStamp, fwKSStamp, hwKernel, fwKernel, bgOrder, nCompKer, kerOrder, usePCA, filter_x, filter_y, temp, PCA, fillVal, mRData);
                if (verbose>=2) fprintf(stderr, "\n");
    ...
    sigma_clip(ss, nss, &mean, &stdev, 10, statSig);
    ...
    for (istamp = 0; istamp < nS; istamp++) {
        ...
        if (stamps[istamp].sscnt < stamps[istamp].nss) {
            ...
            if ((stamps[istamp].chi2 - mean) > kerSigReject * stdev) { 
                ...
                stamps[istamp].sscnt++;
                scnt += (!(fillStamp(&stamps[istamp], imConv, imRef, rPixX, rPixY, verbose, ngauss, deg_fixe, hwKSStamp, fwKSStamp, hwKernel, fwKernel, bgOrder, nCompKer, kerOrder, usePCA, filter_x, filter_y, temp, PCA, fillVal, mRData)));
                if (verbose>=2) fprintf(stderr, "\n");
...
void spatial_convolve(float *image, float **variance, int xSize, int ySize, double *kernelSol, float *cRdata, int *cMask, int kcStep, int hwKernel, int fwKernel, double *kernel, double *kernel_coeffs, int convolveVariance, float kerFracMask, int *mRData, int rPixX, int rPixY, int nCompKer, int kerOrder, double **kernel_vec) {
    /*****************************************************
    ...
    for (j1 = 0; j1 < nsteps_y; j1++) {
        ...
        for(i1 = 0; i1 < nsteps_x; i1++) {
            ...
            make_kernel(i0 + hwKernel, j0 + hwKernel, kernelSol, rPixX, rPixY, nCompKer, kerOrder, fwKernel, kernel_vec, kernel_coeffs, kernel);
...
double make_kernel(int xi, int yi, double *kernelSol, int rPixX, int rPixY, int nCompKer, int kerOrder, int fwKernel, double **kernel_vec, double *kernel_coeffs, double *kernel) {
    /*****************************************************
...
double get_background(int xi, int yi, double *kernelSol, int nCompKer, int kerOrder, int bgOrder, int rPixX, int rPixY) {
    /*****************************************************
...
void make_model(stamp_struct *stamp, double *kernelSol, float *csModel, int rPixX, int rPixY, int nCompKer, int kerOrder, int fwKSStamp) {
    /*****************************************************
