...
/* armin */ 
void loadxyfile(char *filename, int cmpfileflag, float **xcmp, float **ycmp, int *Ncmp){      
    FILE *xyfile;
    int Nalloc,c;
    char line[SCRLEN];
...
/*armin*/
void savexy(stamp_struct *stamps, int nStamps, long xmin, long ymin, int regioncounter, char *xyfilename, int fwKSStamp){
    int  sscnt,istamp;
...
int allocateStamps(stamp_struct *stamps, int nStamps, int bgOrder, int nCompKer, int fwKSStamp, int nC, int nKSStamps) {
    int i,j;
    int nbgVectors;
...
void buildStamps(int sXMin, int sXMax, int sYMin, int sYMax, int *niS, int *ntS,
                 ...
                 int verbose, char *forceConvolve, int rPixX, int rPixY,
                 float tUKThresh, float iUKThresh, int hwKSStamp,
                 int fwStamp, int nKSStamps, float kerFitThresh,
                 int *mRData, float statSig) {
    ...
    if (!(strncmp(forceConvolve, "i", 1)==0)) {
        ...
        if (ctStamps[*ntS].nss == 0) {
            ...
            if ( !( getStampStats3(refArea, ctStamps[*ntS].x0, ctStamps[*ntS].y0, sPixX, sPixY,
                                   ...
                                   &ctStamps[*ntS].lfwhm, 0x0, 0xffff, 3, rPixX, mRData, statSig))) {
                /* pointless */
                if (verbose >= 1)
    ...
    if (!(strncmp(forceConvolve, "t", 1)==0)) {
        ...
        if (ciStamps[*niS].nss == 0) {
            ...
            if ( !( getStampStats3(refArea, ciStamps[*niS].x0, ciStamps[*niS].y0, sPixX, sPixY,
                                   ...
                                   &ciStamps[*niS].lfwhm, 0x0, 0xffff, 3, rPixX, mRData, statSig))) {
    ...
    if (!(strncmp(forceConvolve, "i", 1)==0)) {
        ...
        if (getCenters) {
            ...
            getPsfCenters(&ctStamps[*ntS], tRData, sPixX, sPixY, tUKThresh, bbitt1, bbitt2, nKSStamps, hwKSStamp, rPixX, mRData, kerFitThresh, verbose);
            if (verbose >= 1)
            ...
            if (nss < nKSStamps) {
                ...
                check = checkPsfCenter(tRData, xmax-ctStamps[*ntS].x0, ymax-ctStamps[*ntS].y0, sPixX, sPixY,
                                       ...
                                       0, 0, bbitt1 | bbitt2 | 0xbf, bbitt1, rPixX, hwKSStamp, mRData, kerFitThresh);
    ...
    if (!(strncmp(forceConvolve, "t", 1)==0)) {
        ...
        if (getCenters) {
            /* get potential centers for the kernel fit */
            getPsfCenters(&ciStamps[*niS], iRData, sPixX, sPixY, iUKThresh, bbiti1, bbiti2, nKSStamps, hwKSStamp, rPixX, mRData, kerFitThresh, verbose);
            if (verbose >= 1)
            ...
            if (nss < nKSStamps) {
                ...
                check = checkPsfCenter(iRData, xmax-ciStamps[*niS].x0, ymax-ciStamps[*niS].y0, sPixX, sPixY,
                                       ...
                                       0, 0, bbiti1 | bbiti2 | 0xbf, bbiti1, rPixX, hwKSStamp, mRData, kerFitThresh);
...
int cutSStamp(stamp_struct *stamp, float *iData, int fwKSStamp, int hwKSStamp, float fillVal, int rPixX, int *mRData, int verbose) {
    /*****************************************************
...
double checkPsfCenter(float *iData, int imax, int jmax, int xLen, int yLen,
                      ...
                      int rPixX, int hwKSStamp, int *mRData, float kerFitThresh) {
...
int getPsfCenters(stamp_struct *stamp, float *iData, int xLen, int yLen, double hiThresh, int bbit1, int bbit2, int nKSStamps, int hwKSStamp, int rPixX, int *mRData, float kerFitThresh, int verbose) {
    /*****************************************************
            ...
            for (i = xbuffer; i < xLen-xbuffer; i++) {
                ...
                if (dpt > loPsf) {
                    ...
                    dmax2 = checkPsfCenter(iData, imax, jmax, xLen, yLen, sx0, sy0,
                                           hiThresh, sky, invdsky, xbuffer, ybuffer, bbit, bbit1, rPixX, hwKSStamp, mRData, kerFitThresh);
...
void getNoiseStats3(float *data, float *noise, double *nnorm, int *nncount, int umask, int smask, int rPixX, int rPixY, int *mRData) {
    /*
...
int getStampStats3(float *data,
                   ...
                   int rPixX, int *mRData, float statSig) {
    /*****************************************************
    ...
    */
    ...
    if (sigma_clip(sdat, goodcnt, mean, sd, maxiter, statSig)) {
        free(sdat);
        free(work);
        return 5;
    }
...
int sigma_clip(float *data, int count, double *mean, double *stdev, int maxiter, float statSig) {
    int cnt, ncnt, i, iter;
    char *smask;
    double istdev;
    float d;
...
float *calculateAvgNoise(float *image, int *mask, int nx, int ny, int size, int maskval, int doavg) {
    ...
    for (j = ny; j--; ) {
        for (i = nx; i--; ) {
            ...
            sigma_clip(data, cnt, &mean, &stdev, 0, 3.0);
...
void freeStampMem(stamp_struct *stamps, int nStamps, int nCompKer, int nBGVectors, int nC) {
    /*****************************************************
    ...
    if (stamps) {
        for (i = 0; i < nStamps; i++) {
            if (stamps[i].vectors) {
                for(j = 0; j < nCompKer + nBGVectors; j++) {
                    if (stamps[i].vectors[j]) { free(stamps[i].vectors[j]); stamps[i].vectors[j] = NULL; }
                }
                free(stamps[i].vectors); stamps[i].vectors = NULL;
            }
            if (stamps[i].mat) {
                for (j = 0; j < nC; j++) {
                    if (stamps[i].mat[j]) { free(stamps[i].mat[j]); stamps[i].mat[j] = NULL; }
                }
                free(stamps[i].mat); stamps[i].mat = NULL;
            }
            if (stamps[i].krefArea) { free(stamps[i].krefArea); stamps[i].krefArea = NULL; }
            if (stamps[i].scprod) { free(stamps[i].scprod); stamps[i].scprod = NULL; }
            if (stamps[i].xss) { free(stamps[i].xss); stamps[i].xss = NULL; }
            if (stamps[i].yss) { free(stamps[i].yss); stamps[i].yss = NULL; }
        }
    }
}

float *makeNoiseImage4(float *iData, float invGain, float quad, int rPixX, int rPixY) {
...
void getKernelInfo(char *kimage, int *ngauss, int *fwKernel, int *kerOrder, int *bgOrder, int **deg_fixe, float **sigma_gauss, char **photNormalize) {
    /*****************************************************
...
void readKernel(char *kimage, int nRegion, double **tKerSol, double **iKerSol,
                ...
                int nCompTotal, char **forceConvolve) {
    ...
    *rYMax -= 1;
    ...
    if (strncmp(hInfo, "TEMPLATE", 8)==0) {
        ...
        *tKerSol = (double *)realloc(*tKerSol, (nCompTotal+1)*sizeof(double));
        ...
        fits_get_kernel_btbl(kPtr, &(*tKerSol), nRegion, nCompTotal);
    }
    else if (strncmp(hInfo, "IMAGE", 5)==0) {
        *forceConvolve = "i";
        *iKerSol = (double *)realloc(*iKerSol, (nCompTotal+1)*sizeof(double));
        
        fits_get_kernel_btbl(kPtr, &(*iKerSol), nRegion, nCompTotal);
    }
...
void fits_get_kernel_btbl(fitsfile *kPtr, double **kernelSol, int nRegion, int nCompTotal) {
...
void makeInputMask(float *tData, float *iData, int *mData, int rPixX, int rPixY, float fillVal, float tUThresh, float iUThresh, float tLThresh, float iLThresh, int hwKernel, float kfSpreadMask1) {
    /*****************************************************/
...
void extract_subregion_flt(float *fullData, long fullNx,
                           int xMin, int yMin, int xMax, int yMax,
                           float *subData, int subNx) {
    int i, j;
    for (j = yMin; j <= yMax; j++)
        for (i = xMin; i <= xMax; i++)
            subData[(i - xMin) + subNx * (j - yMin)] = fullData[i + fullNx * j];
}

void extract_subregion_int(int *fullData, long fullNx,
                           int xMin, int yMin, int xMax, int yMax,
                           int *subData, int subNx) {
    int i, j;
    for (j = yMin; j <= yMax; j++)
        for (i = xMin; i <= xMax; i++)
            subData[(i - xMin) + subNx * (j - yMin)] = fullData[i + fullNx * j];
...
void insert_subregion_flt(float *subData, int subNx,
                          float *fullData, long fullNx,
                          int fpixelX, int fpixelY,
                          int lpixelX, int lpixelY,
                          int xBufLo, int yBufLo) {
    int x, y;
    int pixX = lpixelX - fpixelX + 1;
    int pixY = lpixelY - fpixelY + 1;
    for (y = 0; y < pixY; y++)
        for (x = 0; x < pixX; x++)
            fullData[(fpixelX - 1 + x) + fullNx * (fpixelY - 1 + y)] =
                subData[(xBufLo + x) + subNx * (yBufLo + y)];
}

void insert_subregion_int(int *subData, int subNx,
                          int *fullData, long fullNx,
                          int fpixelX, int fpixelY,
                          int lpixelX, int lpixelY,
                          int xBufLo, int yBufLo) {
    int x, y;
