...
def buildAllKernels(kernelSol: np.ndarray, kernelVec2d: np.ndarray, nCompKer: int, kerOrder: int, fwKernel: int, hwKernel: int, kcStep: int, rPixX: int, rPixY: int, xSize: int, ySize: int) -> Tuple[np.ndarray, int, int]:
    ...
    xi = (iGrid + hwKernel).ravel().astype(np.float32)
    yi = (jGrid + hwKernel).ravel().astype(np.float32)
    xf = (xi - halfX) / halfX
    yf = (yi - halfY) / halfY

    kernelCoeffs = np.zeros((nBlocks, nCompKer), dtype=np.float32)
    kernelCoeffs[:, 0] = kernelSol[1]

    k = 2
    for ig in range(1, nCompKer):
        coeff = np.zeros(nBlocks, dtype=np.float32)
        ax = np.ones(nBlocks, dtype=np.float32)
        for ix in range(kerOrder + 1):
            ay = np.ones(nBlocks, dtype=np.float32)
...
def spatial_convolve_jit_kernel(
    ...
    # kernel = np.zeros(fwSq, dtype=np.float32)  # moved inside prange
    # kernel_coeffs = np.zeros(nCompKer, dtype=np.float32)  # moved inside prange
    ...
    for j1 in numba.prange(nsteps_y):
        ...
        for i1 in range(nsteps_x):
            ...
            if allKernels is not None:
                ...
                kernel = np.zeros(fwSq, dtype=np.float32)
...
def background_loop_jit(oRData1d: np.ndarray, kernelSol: np.ndarray, nCompKer: int, kerOrder: int, bgOrder: int, rPixX: int, rPixY: int, hwKernel: int) -> None:
    ...
    halfX = np.float32(0.5 * rPixX)
    halfY = np.float32(0.5 * rPixY)
...
def kernel_vector_pca_numpy(n: int, fwKernel: int, PCA: np.ndarray, kernel_vec: List[np.ndarray]) -> Tuple[np.ndarray, int]:
    ...
    """
    vector = np.zeros(fwKernel * fwKernel, dtype=np.float32)
...
def build_matrix_jit(all_mat: np.ndarray, all_vectors: np.ndarray, valid_mask: np.ndarray, all_x: np.ndarray, all_y: np.ndarray,
    ...
    for istamp in range(nS):
        ...
        xstamp = all_x[istamp]
        ystamp = all_y[istamp]
        fx = np.float32((np.float32(xstamp) - rPixX2) / rPixX2)
        fy = np.float32((np.float32(ystamp) - rPixY2) / rPixY2)
...
def build_matrix_numpy(saMat: np.ndarray, saVectors: np.ndarray, saSscnt: np.ndarray, saNss: np.ndarray, saXss: np.ndarray, saYss: np.ndarray, nS: int, nCompKer: int, kerOrder: int, bgOrder: int, fwKSStamp: int, rPixX: int, rPixY: int, nKSStamps: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # nC = sa.nC
    nC_valid = nCompKer + 1
    # all_mat = np.zeros((nS, nC_valid, nC_valid), dtype=np.float32)
    all_mat = saMat[:, :nC_valid, :nC_valid].copy()
    nvec_total = nCompKer + nbg_vec
    # all_vectors = np.zeros((nS, nvec_total, pixStamp), dtype=np.float32)
...
def build_scprod_numpy(saScprod: np.ndarray, saVectors: np.ndarray, saSscnt: np.ndarray, saNss: np.ndarray, saXss: np.ndarray, saYss: np.ndarray, nS: int, image: np.ndarray, nCompKer: int, kerOrder: int, bgOrder: int, fwKSStamp: int, hwKSStamp: int, rPixX: int, wxy: np.ndarray, nKSStamps: Optional[int] = None) -> np.ndarray:
    ...
    # all_scprod = np.zeros((nS, nCompKer + 1), dtype=np.float32)
...
def fill_stamp_numba_kernel_local(
    ...
    kernels = np.zeros((nvec, fwSqKernel), dtype=np.float32)
    for n in range(nvec):
...
def fill_stamp_numba(saXss: np.ndarray, saYss: np.ndarray, saSscnt: np.ndarray, saNss: np.ndarray, si_list: List[int], imConv: np.ndarray, imRef: np.ndarray, rPixX: int, rPixY: int, ngauss: int, deg_fixe: List[int],
    ...
    img_flat = np.asarray(imConv, dtype=np.float32).ravel()
    imRef_flat = np.asarray(imRef, dtype=np.float32).ravel()
    fx = np.asarray(filter_x, dtype=np.float32)
    fy = np.asarray(filter_y, dtype=np.float32)
    ...
    out_vectors = np.zeros((n_stamps, nvec + nbg, fwSqStamp), dtype=np.float32)
    out_krefArea = np.zeros((n_stamps, fwSqStamp), dtype=np.float32)
    out_mat = np.zeros((n_stamps, nC + 1, nC + 1), dtype=np.float32)
    out_scprod = np.zeros((n_stamps, nC + 1), dtype=np.float32)
    out_sum_val = np.zeros((n_stamps, 1), dtype=np.float32)
...
def get_stamp_sig_batch_jit(
    ...
    for si in range(nS):
        ...
        nsig = 0
        sig1 = 0.0
        temp = np.zeros(fwSq, dtype=np.float32)
        for j in range(fwKSStamp):
...
def spatial_convolve_fast_numpy(image: np.ndarray, variance: np.ndarray, xSize: int, ySize: int, kernelSol: np.ndarray, cMask: np.ndarray, kcStep: int,
    ...
    """快速空间域卷积：预计算所有 kcStep 块的核，调用 spatial_convolve_jit_kernel 一次遍历完成卷积、variance 计算和 mask 标记。将图像和 variance 转换为 1D float32 数组，调用 buildAllKernels 预计算块核，然后启动 numba jit kernel 并行处理。
    ...
    image1d = image.astype(np.float32).ravel()
    cMask1d = cMask.astype(np.int32).ravel()

    cRdata64 = np.zeros(xSize * ySize, dtype=np.float32)
    mRData64 = np.zeros(xSize * ySize, dtype=np.int32)

    if dovar:
        var1d = variance.astype(np.float32).ravel()
    else:
        var1d = np.zeros(1, dtype=np.float32)

    vData = None
    vData64 = np.zeros(xSize * ySize, dtype=np.float32)

    kernel_vec_2d = np.array([np.asarray(kernel_vec[idx][:fwSq], dtype=np.float32)
                              for idx in range(nCompKer)])


    allKernels, nstepsX, nstepsY = buildAllKernels(
        kernelSol.astype(np.float32), kernel_vec_2d,
        nCompKer, kerOrder, fwKernel, hwKernel, kcStep,
        rPixX, rPixY, xSize, ySize)

    spatial_convolve_jit_kernel(image1d, var1d, cMask1d,
        cRdata64, vData64, mRData64, kernelSol.astype(np.float32),
...
def check_stamps_numpy(saScprod: np.ndarray, saMat: np.ndarray, saNorm: np.ndarray, saDiff: np.ndarray, saSscnt: np.ndarray, saNss: np.ndarray, saXss: np.ndarray, saYss: np.ndarray,
    ...
    for i in range(nS):
        check_mat = np.zeros((nComps + 1, nComps + 1), dtype=np.float32)
        check_vec = np.zeros(nComps + 1, dtype=np.float32)
    ...
    if forceConvolve[0:1] == "b":
        ...
        testKerSol = np.zeros(ncomp + nbg_vec + 2, dtype=np.float32)

        wxy = np.zeros((ntestStamps, ncomp2), dtype=np.float32)
        ...
        kernel_coeffs = np.zeros(nCompKer, dtype=np.float32)
        kernel_arr = np.zeros(fwKernel * fwKernel, dtype=np.float32)
        ...
        for i in range(ntestStamps):
            ...
            im = testKrefArea[i]
            vectors = np.asarray(testVectors[i], dtype=np.float32)
            kSol = np.asarray(testKerSol, dtype=np.float32)
            imNoiseArr = np.asarray(imNoise, dtype=np.float32)
...
def check_again_numpy(saSscnt: np.ndarray, saNss: np.ndarray, saChi2: np.ndarray, saXss: np.ndarray, saYss: np.ndarray, saVectors: np.ndarray, saKrefArea: np.ndarray, kernelSol: np.ndarray,
    ...
    if figMerit[0:1] == "v":
        ...
        ncompBG = (nCompKer - 1) * (((kerOrder + 1) * (kerOrder + 2)) // 2) + 1
        batched_bg = np.zeros(nS, dtype=np.float32)
        kb = 1
        ax_arr = np.ones(nS, dtype=np.float32)
        for i in range(bgOrder + 1):
            ay_arr = np.ones(nS, dtype=np.float32)
            for j in range(bgOrder - i + 1):
        ...
        batched_coeffs4 = np.zeros((nS, nCompKer), dtype=np.float32)
        kk = 2
        for i1 in range(1, nCompKer):
            coeff_arr = np.zeros(nS, dtype=np.float32)
            ax_arr = np.ones(nS, dtype=np.float32)
            for ix in range(kerOrder + 1):
                ay_arr = np.ones(nS, dtype=np.float32)
                for iy in range(kerOrder - ix + 1):
        ...
        get_stamp_sig_batch_jit(
            np.asarray(saVectors, dtype=np.float32), np.asarray(saKrefArea, dtype=np.float32),
            saSscnt, saNss, saXss, saYss,
            np.asarray(kernelSol, dtype=np.float32),
            np.asarray(imNoise, dtype=np.float32),
    ...
    for istamp in range(nS):
        if sscnt_local[istamp] < saNss[istamp]:
            if batch_sig1 is not None:
                ...
                vectors = np.asarray(saVectors[istamp], dtype=np.float32)
                kSol = np.asarray(kernelSol, dtype=np.float32)
                imNoiseArr = np.asarray(imNoise, dtype=np.float32)
                mRDataArr = np.asarray(mRData, dtype=np.int64)
                imArr = np.asarray(im, dtype=np.float32)
